...
class FlowersController {
  ...
  update = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const id: any = req.params.flowerId;
      const flower = await FlowerModel.build(req.body);
      const data = await this._validateData(flower, id);
  ...
  _validateData = async (data: FlowerModel, id?: number) => {
    ...
    if (await this._checkIfFlowerExists(flower.name: string, id: number)) {
  ...
  _checkIfFlowerExists = async (name: string, id: number) => {
    const where: any = {
      name: name
    };
...
export default new FlowersController();
