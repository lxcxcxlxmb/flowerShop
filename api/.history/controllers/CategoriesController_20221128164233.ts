...
class CategoriesController {
  ...
  update = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const category = await CategoryModel.build(req.body);
      const data = await this._validateData(category, id);
  ...
  _validateData = async (data: CategoryModel, id?: number) => {
    if (!data.description) {
      throw new Error(`Description is required.`);
    }

    if (await this._checkIfDescriptionExists(data.description, id)) {
      throw new Error(`Category "${data.description}" already exists.`);
    }
